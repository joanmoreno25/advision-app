...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (error.code === 'auth/email-already-in-use') {
        ...
        setErrorMsg("Error al crear la cuenta. Inténtalo de nuevo.");
      }
    }
  };
  ...
  return (
    <>
      <Helmet>
        <title>Crear Cuenta | AdVision</title>
        <meta name="description" content="Regístrate en AdVision para gestionar y analizar tus archivos publicitarios con IA." />
      </Helmet>
      ...
      <div className="min-h-screen flex w-full font-sans bg-white">
        ...
        <div className="w-full lg:w-1/2 flex flex-col justify-center px-[8%] sm:px-[12%] lg:px-[15%] py-12">
          ...
          <div className="flex flex-col items-center mb-10 mt-4">
            ...
            <h1 className="text-[#0F172A] text-[40px] font-extrabold mb-2 text-center leading-[1.1]">Crear cuenta</h1>
            <p className="text-[#64748B] text-[18px] text-center">Introduce tus datos para empezar</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="flex flex-col gap-5">
            <div>
              <label className="block text-[#0F172A] text-[16px] font-semibold mb-2">Nombre Completo</label>
              <input 
                type="text" 
                value={name}
            ...
            <div>
              <label className="block text-[#0F172A] text-[16px] font-semibold mb-2">Repetir contraseña</label>
              <div className="relative">
                <input 
            ...
            <div className="flex items-center gap-3 mt-2">
              <input 
                ...
                className="w-4 h-4 rounded border-[#CBD5E1] text-[#2563EB] focus:ring-[#2563EB] flex-shrink-0"
              />
              <label htmlFor="terms" className="text-[#64748B] text-[15px] leading-none mt-px">
                Acepto los <Link to="/terms" className="text-[#2563EB] font-medium hover:underline">Términos y Condiciones</Link>
              </label>
            </div>

            <button 
              type="submit" 
              className="w-full bg-[#2563EB] text-white text-[18px] font-bold px-4 py-4 rounded-[10px] hover:bg-blue-700 transition-colors mt-4 shadow-md active:scale-[0.98]"
            >
              Crear cuenta
            </button>
          </form>
          ...
          <p className="text-center text-[#64748B] text-[16px] mt-8">
            ¿Ya tienes cuenta? <Link to="/login" className="text-[#2563EB] font-semibold hover:underline">Iniciar sesión</Link>
          </p>
        ...
        <div className="hidden lg:flex flex-col lg:w-1/2 bg-[#2563EB] relative items-center justify-center p-12 overflow-hidden">
          ...
          <h2 className="relative z-10 text-white text-[42px] xl:text-[48px] font-extrabold tracking-tight text-center leading-[1.1]">
            ¡Únete a nosotros!
          </h2>
        </div>
